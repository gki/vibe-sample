...
以下のようなコードはテスト対象から除外します：

- **自動生成されたコード**: Prismaクライアント、GraphQLスキーマの自動生成部分など
- **外部ライブラリの動作**: ライブラリ自体の動作をテストするもの（例: `buildSchema`が正しく動作すること）
- **実装コードをテストしていないテスト**: 実際の実装コードを使わず、独自に定義したコードでライブラリの動作を確認するだけのテスト

**理由**: これらのコードは開発者が書いたコードではなく、ライブラリやツールが生成したコード、またはライブラリ自体の動作をテストするもののため、プロジェクトのテスト対象外とします。実際の実装コード（リゾルバー、バリデーション関数など）のテストに注力します。
